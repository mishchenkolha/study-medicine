...
// start helpers
const generatePDF = async (username, courseTitle, certificateUrl) => {
    const marginX = 50;
    const doc = new pdfkit_1.default({ size: 'A4', margin: marginX });
    const chunks = [];
    ...
    doc.opacity(1);
    const qrDataUrl = await qrcode_1.default.toDataURL(certificateUrl, {
    ...
    doc
        .fontSize(16)
        .font('Helvetica')
        .text(`has successfully completed the course "${courseTitle}" organized by "Toronto medicine center".`, marginX, currentY, { align: 'center', width: pageWidth - 2 * marginX });
    currentY += 60;
    doc
        .fontSize(14)
...
const getUniqueNames = (username, courseTitle) => {
...
exports.default = {
    async sendCertificate(ctx) {
        ...
        const { courseId: courseDocumentId, title: courseTitle } = ctx.request.body || {};
        ...
        if (existing) {
            return ctx.send({
                isNew: false,
                slug: (_a = existing.slug) !== null && _a !== void 0 ? _a : null,
            });
        }
        try {
            // 3. Унікальні імена та slug
            const { slug, fileName, uniqueTitle } = getUniqueNames(username, courseTitle);
            const certificateUrl = `${process.env.FRONTEND_URL}/certificate/${slug}`;
            // 3.1. Генеруємо pdf
            const pdfBuffer = await generatePDF(username, courseTitle, certificateUrl);
            if (!pdfBuffer.length)
                throw new Error('PDF не згенерувався');
            ...
            if (!apiToken)
                throw new Error('Не заданий STRAPI_API_TOKEN');
            ...
            if (!uploadRes.ok) {
                ...
                throw new Error(`Upload failed: ${uploadRes.status} ${uploadRes.statusText} — ${errText}`);
            }
            const uploadedJson = await uploadRes.json();
            const uploadedFile = Array.isArray(uploadedJson)
                ? uploadedJson[0]
                : uploadedJson;
            const fileDocumentId = uploadedFile === null || uploadedFile === void 0 ? void 0 : uploadedFile.id;
            if (!fileDocumentId)
            ...
            const certificateEntry = await strapi
                ...
                data: {
                    title: uniqueTitle,
                    slug,
                    user: userDocumentId !== null && userDocumentId !== void 0 ? userDocumentId : { id: userId },
            ...
            return ctx.send({
                isNew: true,
                slug: (uploadedFile === null || uploadedFile === void 0 ? void 0 : uploadedFile.url) ? slug : null,
            });
        }
        catch (err) {
            strapi.log.error('Помилка при відправці сертифікату:', err);
            return ctx.internalServerError((err === null || err === void 0 ? void 0 : err.message) || 'Не вдалося відправити сертифікат');
        }
    },
};
