...
exports.default = {
    async submit(ctx) {
        ...
        const { quizDocumentId, courseDocumentId, answers, score, isPassed: passed, } = ctx.request.body;
        if (!quizDocumentId ||
            !courseDocumentId ||
            typeof score !== 'number' ||
            !answers) {
            return ctx.badRequest('quizDocumentId, courseDocumentId, answers і score обов’язкові');
        }
        const entryData = {
            quiz: quizDocumentId,
        ...
        const sanitizedEntry = await strapi.contentAPI.sanitize.input(entryData, resultsContentType);
        try {
    ...
    async latest(ctx) {
        ...
        try {
            ...
            return latestResult !== null && latestResult !== void 0 ? latestResult : {};
        }
        catch (error) {
